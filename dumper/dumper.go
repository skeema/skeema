...
func getStatementMap(schema *tengo.Schema, dir *fs.Dir, opts Options) map[tengo.ObjectKey]statement {
	...
	for key, canonicalCreate := range schemaObjects {
		...
		if key.Type == tengo.ObjectTypeTable && opts.Partitioning != tengo.PartitioningPermissive {
			dbCreateBase, _ := tengo.ParseCreatePartitioning(s.canonicalCreate)
			if opts.Partitioning == tengo.PartitioningKeep && s.fsStatement != nil {
				...
				s.canonicalCreate = dbCreateBase
			}
		}
