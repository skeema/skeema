...
Skeema is currently in public alpha. Many edge cases are not yet supported, but are coming soon.

The `skeema` binary is being tested on macOS and Linux. For now, it cannot be compiled on Windows. On the database side, testing has primarily been performed against MySQL 5.6, MariaDB 10.1, and Percona Server 5.6, all running on Linux.
