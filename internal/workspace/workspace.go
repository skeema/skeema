...
func ExecLogicalSchema(logicalSchema *fs.LogicalSchema, opts Options) (_ *Schema, retErr error) {
	...
	// Create a connection pool: if CREATE chunking is requested (sending batches
	// of multiple CREATE TABLEs in one round-trip), this attempts to enable
	// multiStatements. Some proxies don't support that though, so if it fails,
	// disable chunking and try again without multiStatements.
	...
	var db *sqlx.DB
	var chunkErr error
	if opts.CreateChunkSize > 1 {
		db, chunkErr = ws.ConnectionPool(params + "&multiStatements=true")
	}
	if db == nil { // either no chunking, or error from attempt with multiStatements
		db, err = ws.ConnectionPool(params)
	}
	if err != nil {
		return nil, fmt.Errorf("Cannot connect to workspace: %w", err)
	} else if chunkErr != nil { // This isn't fatal; we just disable chunking
		log.Warnf("Unable to improve workspace performance using multi-statement support: %s\nThis may be caused by a proxy server which doesn't support multi-statements. If so, consider reconfiguring Skeema to bypass the proxy in order to improve performance.", err)
		opts.CreateChunkSize = 1
	}
	wsSchema.Timers.Init = time.Since(timerStart)

	// With chunking enabled, tables must be separated from non-tables, since
